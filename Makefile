...
DOCKER_IMAGE_TAG=$(REPO)/$(RELEASE_NAME):$(LATEST_TAG)
DAPR_RUNTIME_DOCKER_IMAGE_TAG=$(DAPR_RUNTIME_DOCKER_IMAGE_TAG):$(LATEST_TAG)
DAPR_PLACEMENT_DOCKER_IMAGE_TAG=$(DAPR_PLACEMENT_DOCKER_IMAGE_TAG):$(LATEST_TAG)
DAPR_SENTRY_DOCKER_IMAGE_TAG=$(DAPR_SENTRY_DOCKER_IMAGE_TAG):$(LATEST_TAG)
endif

check-arch-platform:
ifeq ($(GOARCH),arm)
  DOCKER_IMAGE_PLATFORM:=$(GOOS)/arm/v7
...
docker-build: check-arch-platform
ifeq ($(GOARCH),amd64)
